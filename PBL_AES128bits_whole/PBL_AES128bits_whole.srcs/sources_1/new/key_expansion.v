...
module key_expansion(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            done<=1'b0; round<=4'd0; round_keys<=1408'd0;
            ...
            case (round)
                4'd0: begin
                    ...
                    if (start) begin
                        ...
                        round_keys[0 +: 128] <= {w[0],w[1],w[2],w[3]};
                        round <= 4'd1;
                    end
                end
                ...
                4'd6,4'd7,4'd8,4'd9,4'd10: begin
                    begin : ks
                        reg [31:0] prev,rotw,subw,t,nw0,nw1,nw2,nw3;
                        integer    b;
                        b    = round * 4;
                        prev = w[b-1];
                        rotw = {prev[23:0],prev[31:24]};
                        subw = {sbox_fn(rotw[31:24]),sbox_fn(rotw[23:16]),
                                sbox_fn(rotw[15:8]), sbox_fn(rotw[7:0])};
                        t    = subw ^ {rcon(round),24'h000000};
                        nw0  = w[b-4]^t;  nw1 = w[b-3]^nw0;
                        nw2  = w[b-2]^nw1; nw3 = w[b-1]^nw2;
                        w[b]=nw0; w[b+1]=nw1; w[b+2]=nw2; w[b+3]=nw3;
                        round_keys[round*128 +: 128] <= {nw0,nw1,nw2,nw3};
                    end
                    if (round==4'd10) begin done<=1'b1; round<=4'd0; end
